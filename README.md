...
- `resolve_token(conn, account_id)` — returns the authentication token. In multi-account mode it finds the entry in `conn.extra.accounts` whose **sanitized** id matches the sanitized `account_id` (canonical lookup key), so a raw `account_id` like `"a.b"` matches an entry stored as `"a.b"` or `"a_b"`. If two or more entries collapse to the same sanitized id (an ambiguous collision), it raises `AirflowException` rather than silently picking one — a deliberate asymmetry with `list_accounts()`, which deduplicates such collisions with a WARNING and keeps the first. In single-account mode it returns `conn.password`. Raises `AirflowException` when the token cannot be resolved.
