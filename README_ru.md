...
- `resolve_cabinet_id(conn_id, account_id)` — возвращает **санитизированный** cabinet id для операции. В режиме нескольких кабинетов (`account_id` задан) возвращает `Account(id=account_id).id`, без чтения подключения. В одиночном режиме лениво читает `conn.login` и возвращает его санитизированную форму (или `None`, если не задан). Санитизация идемпотентна, поэтому значение, уже пришедшее из `list_accounts()`, не меняется.
- `resolve_token(conn, account_id)` — возвращает токен аутентификации. В режиме нескольких кабинетов ищет первую запись в `conn.extra.accounts`, чей **санитизированный** id совпадает с санитизированным `account_id` (канонический ключ поиска), поэтому сырой `account_id` вида `"a.b"` находит запись, сохранённую как `"a.b"` или `"a_b"`. В одиночном режиме возвращает `conn.password`. Пробрасывает `AirflowException`, если токен не удалось получить.
