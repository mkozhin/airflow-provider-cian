...
| Login | **Account ID** — метка кабинета (**обязательно**) |
...
`Login` **обязателен**: он записывается в каждую запись выгрузки полем `account_id`
и в путь файла как сегмент кабинета. Без него таск падает с `AirflowException` до
запроса к API. Значение — это метка, которую вы выбираете сами (API Cian не выдаёт
реальный ID кабинета); удобно взять ИНН или название юрлица из договора.
...
`id` — **метка, которую вы выбираете сами**, однозначно идентифицирующая кабинет: API Cian не выдаёт реальный ID кабинета, поэтому это не значение из Cian (удобно взять ИНН или название юрлица из договора). Не алфавитно-цифровые символы санитизируются (`[^\w-]` → `_`) единообразно всюду, где используется `id`: в путях к файлам, именах таблиц BigQuery, при поиске токена и в поле `account_id`, которое пишется в каждую запись выгрузки.
...
Путь к файлу всегда содержит сегмент кабинета (санитизированный Account ID):

| `account_id` | `conn.login` | Путь |
|---|---|---|
| не задан | не задан | **ошибка** — `AirflowException` (Account ID обязателен) |
| не задан | `"123"` | `{base_dir}/123/{run_id}/{date}.{ext}` |
| `"123"` | любой | `{base_dir}/123/{run_id}/{date}.{ext}` |

В однокабинетном режиме поле `Login` подключения **обязательно** и задаёт Account ID и для пути файла, и для поля `account_id`. Если не заданы ни `account_id`, ни `Login`, таск падает с `AirflowException` до запроса к API.

### Схема данных

Базовая схема (19 полей) — присутствует во всех записях независимо от формата вывода:
...
- `account_id` — санитизированный Account ID кабинета, которому принадлежит запись (второе поле, сразу после `id`); совпадает с сегментом пути файла. Позволяет отличать записи разных кабинетов в самих данных, а не только по пути файла
- `date` — дата сбора (`YYYY-MM-DD`), всегда равна параметру `date` оператора; безопасна для партиционирования BigQuery по дате
- `datetime` — исходное datetime из API с явным московским смещением (`YYYY-MM-DDTHH:MM:SS+03:00`); доли секунды из ответа API отбрасываются усечением (не округлением), поэтому точность всегда до секунды
- `is_targeted` вычисляется: `billing_price > 0`.

При `add_snapshot_ts=True` и `output_format='json'` каждая запись также содержит 20-е поле:
...
> **BigQuery:** базовая схема `BQ_SCHEMA` в примерах v1 и v2 (`bq_and_s3_dag.py`, `bq_and_s3_dag_v2.py`) фиксирована на **19 полях** (включая `account_id`). Multi-account пример (`bq_and_s3_multi_account_dag.py`) использует `add_snapshot_ts=True`, поэтому его схема — **20 полей** (базовые 19 + `snapshot_ts`). При включении `add_snapshot_ts=True` на 19-польной схеме потребуется либо добавить колонку `snapshot_ts STRING`, либо использовать `ignore_unknown_values=True` в задании загрузки — иначе BigQuery отклонит записи с дополнительным полем.

> **Только для JSON:** `add_snapshot_ts=True` не влияет на вывод в CSV. Схема CSV остаётся 19-польной.
...
- `resolve_cabinet_id(conn_id, account_id)` — возвращает **санитизированный** cabinet id для операции. В режиме нескольких кабинетов (`account_id` задан) возвращает `sanitize_id(account_id)`, без чтения подключения. В одиночном режиме лениво читает `conn.login` и возвращает его санитизированную форму (или `None`, если не задан). Санитизация идемпотентна, поэтому значение, уже пришедшее из `list_accounts()`, не меняется.
- `resolve_token(conn, account_id)` — возвращает токен аутентификации. В режиме нескольких кабинетов ищет запись в `conn.extra.accounts`, чей **санитизированный** id совпадает с санитизированным `account_id` (канонический ключ поиска), поэтому сырой `account_id` вида `"a.b"` находит запись, сохранённую как `"a.b"` или `"a_b"`. Если две или более записи схлопываются в один санитизированный id (неоднозначная коллизия), пробрасывает `AirflowException`, а не молча выбирает одну — намеренная асимметрия с `list_accounts()`, который дедуплицирует такие коллизии с WARNING и оставляет первую. В одиночном режиме возвращает `conn.password`. Пробрасывает `AirflowException`, если токен не удалось получить.
...
Запись, у которой поле `date` отсутствует или не является строкой, валит таск с `AirflowException` с указанием `id` записи. Строку `date`, которую стандартная библиотека не может разобрать, таск отвергает так же — `AirflowException: Record id=<id> has an unparsable 'date': '<значение>'`, — причём `<значение>` цитируется дословно в том виде, в каком его прислал API, до нормализации (`Z` → `+00:00`, отбрасывание долей секунды). Благодаря этому сломавшая парсинг разрядность или формат читаются прямо из лога, а нечитаемое время падает громко, а не «чинится» молча (см. `docs/adr/0005-datetime-drop-subsecond.md`).

Повторы: при HTTP 429 или 5xx — экспоненциальный backoff (1s, 2s, 4s, всего 3 попытки), затем `AirflowException`.
